            ...
            <div class="box box-primary box-solid">
                <div class="box-header with-border">
                    ...
                    <div class="box-tools pull-left" style="float: left">
                        <button type="button"  class="btn btn-box-tool" title="点击可后退">
                            <i class="fa fa-reply"></i></button>
                        <button type="button" class="btn btn-box-tool" title="点击可前进">
                        ...
                        <c:forEach items="${diskList}" var="disk">
                            <tr>
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${disk.type=='file'}">
                                            <a href="/disk/download?id=${disk.id}"> ${disk.sourceName}</a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="/disk?path=${disk.id}">${disk.sourceName}</a>
                                        </c:otherwise>
                                    </c:choose>
    ...
    $(function(){
        ...
        $("#newFolder").click(function(){
            ...
            layer.prompt({title:"请输入文件夹名称"},function(text,index){
                layer.close(index);
                $.post("/disk/folder/new",{"fid":fid,"sourceName":text})
                    .done(function(resp){
                        if(resp.status=='success'){
                            window.history.go(0);
                        }
        ...
        $(".remove").click(function(){
            ...
            layer.confirm("你确定要删除吗？",function(index){
                    layer.close(index);
                $.get("/disk/del/"+id).done(function(resp){
                    if(resp.status=='success'){
                        layer.msg("删除成功");
                        window.history.go(0);
                    }else{
